...
5. Open a PR against `main` with a one-line entry added to the matching category in the README. If the prompt takes on a big, risky task, mark its entry with the violet [spec] badge (`![spec](docs/media/spec-badge.svg)`) and bump the spec-count badge on the category's Contents line.
